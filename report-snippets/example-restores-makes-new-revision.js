...
const db = new Dropbox({ accessToken: "your-access-token-here" })
...
;(async () => {
    ...
    for (const version of ["r1", "r2", "r3"]) {
        await db.filesUpload({ path: remoteFilePath, mode: "overwrite", contents: version })
    }
    // downloads and prints the 3 revisions
    const entries1 = (await db.filesListRevisions({ path: remoteFilePath, mode: "path" })).result.entries
    console.log(entries1.map(entry => entry.rev)) // ['015d70627251361...', '015d706271a07ac...', '015d706270b981f...']

    // Restore the file back to "r2"
    await db.filesRestore({ path: remoteFilePath, rev: entries1[1].rev })

    // downloads the current file and prints it
    const response = await db.filesDownload({ path: remoteFilePath })
    console.log(response.result.fileBinary.toString()) // "r2"

    // downloads and prints the 4 revisions
    const entries2 = (await db.filesListRevisions({ path: remoteFilePath, mode: "path" })).result.entries
