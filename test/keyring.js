...
const { CAPABILITY_TYPE_WRITE, CAPABILITY_TYPE_READ, CAPABILITY_TYPE_VERIFY } = require("../constants")
...
describe("Key Ring", function () {
    ...
    it("should accept adding a valid key-object", async function () {
        ...
        const actual = await kr.getCapabilityWithPathAndType(join(testDirPath, "file1"), CAPABILITY_TYPE_WRITE, keytype)
    ...
    it("should remove key object", async function () {
        ...
        await kr.addCapability(cap)
        const keytype = "string"
        const capabilityActual = await kr.getCapabilityWithPathAndType(path, CAPABILITY_TYPE_WRITE, keytype)
        assert.isNotNull(capabilityActual)

        await kr.removeCapability(cap)
        const k1ActualAfterRemoval = await kr.getCapabilityWithPathAndType(path, CAPABILITY_TYPE_WRITE, keytype)
    ...
    it("should remove key object by path and optional type", async function () {
        ...
        const actual = await kr.getCapabilityWithPathAndType(path, CAPABILITY_TYPE_VERIFY)
    ...
    it("should remove key object by path and optional type", async function () {
        ...
        await kr.removeCapabilitiesWithPath(path, CAPABILITY_TYPE_WRITE)

        const keytype = "string"
        const actualWrite = await kr.getCapabilityWithPathAndType(path, CAPABILITY_TYPE_WRITE, keytype)
    ...
    it("should only update the right key object with new key", async function () {
        ...
        await kr.addCapability(cap1)
        await kr.addCapability(cap2)
        await kr.updateCapabilityKey(path, CAPABILITY_TYPE_WRITE, newWriteKey)
        ...
        const actualVerify = await kr.getCapabilityWithPathAndType(path, CAPABILITY_TYPE_VERIFY, "string")
