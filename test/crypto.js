    ...
    describe("Symmetric Stream Cryptography", function() {
        ...
        describe("#encryptSlice(cipher, nonce, key, buffer, position, length)", function() {
            ...
            it("'position' is at most cipher.length", function() {
                ...
                assert.doesNotThrow(() => crypto.encryptSlice(c, n, k, b, c.length - 1, b.length))
                assert.doesNotThrow(() => crypto.encryptSlice(c, n, k, b, c.length, b.length))
                assert.throws(() => crypto.encryptSlice(c, n, k, b, c.length + 1, b.length))
            })
